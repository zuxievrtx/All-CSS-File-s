...
(function (jQuery) {
  ...
  if (header !== null) {
    ...
    Array.from(headerActiveItem, (elem) => {
      if (elem.closest(".sub-menu") !== null) {
        const childMenu =
          elem.closest(".current-menu-item").parentElement.parentElement;
  ...
  if (document.querySelector("header").classList.contains("has-sticky")) {
    window.addEventListener("scroll", function () {
      ...
      if (document.documentElement.scrollTop > height) {
        document
          .querySelector(".has-sticky .logo")
          .classList.add("logo-display");
      } else if (document.documentElement.scrollTop <= height) {
        document
          .querySelector(".has-sticky .logo")
          .classList.remove("logo-display");
      }
    });
  }
  ...
  const alertButton = document.querySelectorAll(
    '[data-alert="sweetalert-product"]'
  );
        ...
        .then((result) => {
          if (result.isConfirmed) {
            btn.querySelector(".btn").innerHTML =
              'View Cart <i aria-hidden="true" class="fas fa-long-arrow-alt-right ms-2"></i>';
            return true;
          } else if (
  ...
  const alertButton1 = document.querySelectorAll(
    '[data-alert="sweetalert-heart"]'
  );
